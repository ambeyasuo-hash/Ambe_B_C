...
| バージョン | 日付 | 主な変更 |
|---|---|---|
| v6.0.3 | 2026-04-21 | WebAuthn の役割を「Config Bundle を復号する鍵を導出するため」に明確化（Section 2.3・3.1・4.4）。初回セットアップ順序をモック準拠に修正（⑨生体認証→③API→⑧PIN→⑩24単語）。Config Bundle に `ambe_generation`・`last_exported_at` フィールドを追加（Section 2.2）。`encryption_salt` の HKDF ドメイン分離設計を明記（Section 4.2）。未分類カテゴリを予約済みID `system-default` で保護（Section 7.2）。OCR 処理中オーバーレイの追加（Section 9.11）。暗号処理は全て `'use client'` で完結するルールを明記（Section 5.2）。 |
...
```json
{
  ...
  "ambe_generation": 1,
  "last_exported_at": "2026-04-21T10:00:00Z",
  "supabase": {
...
| 項目 | 役割 | 必須 |
|---|---|---|
| `encryption_salt` | ユーザー識別子（Supabase の行特定・HKDF のベース） | ✅ |
| `ambe_generation` | .ambe エクスポートの世代番号（エクスポートごとに+1） | ✅ |
| `last_exported_at` | 最終エクスポート日時（衝突検知の補助） | ✅ |
...
**`ambe_generation` の世代管理ルール**:
- **エクスポート時**: 現在の `ambe_generation` に +1 した値をファイルに書き込み、同時に localStorage の Config Bundle も更新する
- **インポート時**: ファイルの `ambe_generation` がアプリ内の値より大きければ上書き。小さければ「古い設定ファイルです」の警告を表示（インポートは可能）
- **初期値**: 初回セットアップ時は `1`
...
> **設計意図**: WebAuthn（生体認証）だけでは、デバイス紛失時にデータ復旧が不可能になる。
> Config Bundle を「WebAuthn 派生鍵で開ける層」と「PIN/リカバリフレーズで開ける層」に
> 分けることで、デバイスに依存しない移行（QR・.ambe）を実現する。
...
```
【Step 1/4】 生体認証キーの登録（⑨）
    WebAuthn platform authenticator を登録
    → assertion 用の credentialId を取得・保持（localStorage）

【Step 2/4】 API 情報入力（③）
    ├─ Supabase URL / anon key  ← 疎通テスト（自動・必須）
    ├─ Azure endpoint / key     ← 疎通テスト（自動・必須）
    └─ Gemini key (任意)        ← 疎通テスト（任意）
    ※ Supabase SQL セットアップブロックをこの画面で提供

【Step 3/4】 PIN 設定（⑧）
    4〜8 桁の PIN を入力・確認
    → PBKDF2-SHA256 (100,000 iterations) で wrapping key 導出
    → Config Bundle を暗号化 → localStorage['config_bundle_wrapped_pin']

【この時点でアプリが自動処理】
    ├─ encryption_salt (UUID v4) 生成
    ├─ Data Key (AES-256 ランダム) 生成
    ├─ Recovery Mnemonic (BIP-39 24 単語) 生成
    ├─ WebAuthn assertion → HKDF で wrapping key 導出
    │   → Config Bundle を暗号化 → localStorage['config_bundle_alpha']
    ├─ Config Bundle を組み立て（ambe_generation=1 含む）
    └─ wrapped_data_key_alpha / beta を Supabase user_vault に保存

【Step 4/4】 24 単語バックアップ（⑩）
    ├─ 24 単語を画面表示
...
[4] ct = AES-256-GCM(Config Bundle, wrapping_key, IV)
[5] QR payload を作成
     {
       "v": 1,
       "kind": "config-bundle",
...
「既存の端末から引き継ぐ」タップ
  ↓
[1] カメラ起動 → QR スキャン
[2] QR payload パース → ct, iv, salt 取得
[3] 画面表示された 6 桁 PIN を入力
[4] wrapping_key = PBKDF2(PIN, salt, 100,000)
[5] Config Bundle = AES-256-GCM-Decrypt(ct, wrapping_key, iv)
[6] 復号成功 → Device B も Config Bundle を保持
  ↓
[7] Device B 自身の保護方式を設定
...
- 中継サーバー不要（Device A → Device B の一方向のみ）
- 5 分で payload が失効
- encryption_salt はそのまま引き継がれ、両端末で同じ Supabase データを参照可能
...
| WebAuthn assertion → wrapping key | HKDF-SHA256 | info="config-bundle-wrapping-key" |
| Blind Indexing | HMAC-SHA256 | HKDF 派生サブキーを使用（後述） |
| Pairing PIN → wrapping key | PBKDF2-SHA256 | 100,000 iterations / salt 16B |

**`encryption_salt` の HKDF ドメイン分離設計**:

`encryption_salt` は Supabase 行特定 ID・HMAC 鍵ベース・24 単語からの決定論的導出の 3 役を担うが、HKDF によるドメイン分離により用途ごとに独立したサブキーを生成する。

```
master_salt = encryption_salt (UUID v4)
              ↓ HKDF-SHA256
hmac_key       = HKDF(master_salt, info="blind-index-hmac",    length=32)  → Blind Indexing 専用
row_id         = encryption_salt そのまま                                   → Supabase 行特定
mnemonic_salt  = HKDF(mnemonic_seed, info="encryption-salt",   length=16)  → 24単語からの復元
```

この設計により、Blind Indexing の HMAC 鍵が Supabase 行 ID と直接一致しないため、ハッシュ値から行 ID を推測することが暗号学的に困難になる。
...
- `authenticatorAttachment`: `'platform'` 固定（USB/NFC キー不可）
...
すべての暗号化・復号ロジックは `'use client'` 指定のコンポーネント・Context・Hooks 内で完結させる。

```
✅ 正しい配置
src/lib/crypto.ts         ← 'use client' 前提のユーティリティ
src/lib/vault.ts          ← 'use client' 前提
src/context/VaultContext.tsx  ← 'use client' + React Context
src/hooks/useVault.ts     ← 'use client' Hook

❌ 禁止
app/api/*/route.ts に CryptoKey を渡す  → シリアライズ不可・Zero-Knowledge 違反
Server Component で crypto 処理         → Vercel サーバーに鍵が送られる
```

> **理由①**: `CryptoKey` オブジェクトは Web Crypto API が JS エンジン内で隔離管理しており、JSON シリアライズ不可。
> **理由②**: Server Component の実行場所は Vercel サーバー。鍵をそこに送った瞬間に Zero-Knowledge が崩壊する。

### 5.3 Security / Crypto

- Web Crypto API（AES-GCM, PBKDF2, HMAC, HKDF）
...
| カラム | 型 | 説明 |
|---|---|---|
| `encrypted_data` | TEXT | PII 一括暗号化 JSON |
...
「未分類」カテゴリは通常レコードとして扱わず、**予約済みID `system-default`** で識別・保護する。

```sql
-- 初回セットアップ時に INSERT（gen_random_uuid() ではなく固定ID）
INSERT INTO categories (id, encryption_salt, name, color_index, sort_order)
VALUES ('system-default', '<encryption_salt>', '未分類', 0, 0)
ON CONFLICT (id) DO NOTHING;

-- RLS: system-default は DELETE 不可
CREATE POLICY "protect system-default category"
  ON categories FOR DELETE TO anon
  USING (id != 'system-default');
```

**UI 制御**: `id === 'system-default'` の行に対しては削除・編集ボタンを非活性化する。

**カテゴリ操作**:
- 追加・編集・削除はすべてクライアント側で実行
- 削除時は紐づく `business_cards.card_category` を `NULL`（→ 表示上「未分類」扱い）に更新
- カテゴリ名は最大 20 文字
...
[1] OCR・構造化（自動）
    ...
    ※ Azure 処理中（数秒）はスキャン画面上に「解析中...」オーバーレイを表示
       → スピナー + 「名刺を解析しています」テキスト
       → 処理完了後にプレビュー画面へ自動遷移
