...
- **AI OCR**: OCR解析のため名刺画像を Azure AI Document Intelligence へ送信し、部署・役職・フリガナなどを自動抽出します
- **AI メール作成**: Gemini でお礼メールのドラフトを自動生成します。個人情報はプレースホルダー化して送信し、生成後に端末内で差し込みます
...
> **AI OCRへの送信**: 名刺画像はOCR解析のため、あなたが設定した Azure AI Document Intelligence リソースへ送信されます。アプリのセットアップ時に、この送信と解析への同意チェックが必要です。
...
> **プライバシー**: Gemini には氏名・会社名・メールアドレス・電話番号・住所などの実データを送信しません。氏名・会社名・部署名・役職は `[[NAME]]` などのプレースホルダーに置き換えて送信し、生成された文章に端末内で復号済み情報を差し込みます。
